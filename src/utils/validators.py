...
class DocumentValidator:
    ...
    def validate_file(
        self, filename: str, file_content: bytes, declared_type: Optional[str] = None
    ) -> ValidationResult:
        """
    ...
    def _validate_filename(self, filename: str, result: ValidationResult):
        ...
        if file_ext not in self.supported_types:
            result.add_error(
                f"Unsupported file type: {file_ext}. Supported: {self.supported_types}",
                "UNSUPPORTED_TYPE",
            )
    ...
    def _validate_file_type(
        ...
        if file_ext == "pdf" and not detected_type.startswith("pdf"):
            result.add_warning(
                f"File extension suggests PDF but content appears to be {detected_type}"
            )
        elif file_ext == "txt" and not detected_type.startswith("text"):
            result.add_warning(
                f"File extension suggests text but content appears to be {detected_type}"
            )

        # Check declared type vs actual
        if declared_type and declared_type != file_ext:
            result.add_warning(
                f"Declared type '{declared_type}' doesn't match extension '{file_ext}'"
            )
    ...
    def _detect_file_type(self, file_content: bytes) -> str:
        ...
        try:
            ...
            if (
                any(char in text_content for char in ["\n", "\r", "\t"])
                or text_content.isprintable()
            ):
                return "text/plain"
    ...
    def _validate_pdf_content(self, file_content: bytes, result: ValidationResult):
        ...
        try:
            ...
            if pdf_reader.is_encrypted:
                result.add_error(
                    "PDF is encrypted and cannot be processed", "ENCRYPTED_PDF"
                )
                return
            ...
            if num_pages == 0:
                ...
                result.add_warning(
                    f"PDF has many pages ({num_pages}) - processing may be slow"
                )
            ...
            if not text_found:
                result.add_warning(
                    "No readable text found in first 5 pages - may be image-based PDF"
                )
            ...
            result.add_error(
                f"Error validating PDF content: {str(e)}", "PDF_VALIDATION_ERROR"
            )
    ...
    def _validate_text_content(self, file_content: bytes, result: ValidationResult):
        ...
        try:
            ...
            if text_length < self.min_content_length:
                result.add_error(
                    f"Text content too short: {text_length} characters",
                    "TEXT_TOO_SHORT",
                )
            ...
            if null_bytes > 0:
                result.add_warning(
                    f"Text contains {null_bytes} null bytes - may be binary data"
                )
            ...
            result.add_error(
                f"Error validating text content: {str(e)}", "TEXT_VALIDATION_ERROR"
            )
...
class EmbeddingValidator:
    ...
    def validate_embedding(
        self, embedding: List[float], context: str = ""
    ) -> ValidationResult:
        """
        ...
        if not isinstance(embedding, (list, tuple)):
            result.add_error(
                f"{context_prefix}Embedding must be a list or tuple", "INVALID_TYPE"
            )
            return result
        ...
        for i, value in enumerate(embedding):
            ...
            if not (
                self.reasonable_value_range[0]
                <= value
                <= self.reasonable_value_range[1]
            ):
        ...
        if zero_count == len(embedding):
            result.add_error(
                f"{context_prefix}All embedding values are zero", "ALL_ZEROS"
            )
        elif zero_count > len(embedding) * 0.9:  # More than 90% zeros
            result.add_warning(
                f"{context_prefix}Embedding is mostly zeros ({zero_count}/{len(embedding)})"
            )
        ...
        if len(embedding) > 0 and all(isinstance(v, (int, float)) for v in embedding):
            ...
            try:
                ...
                if abs(mean_val) > 1.0:
                    result.add_warning(
                        f"{context_prefix}Unusual mean value: {mean_val:.4f}"
                    )

                if stdev_val < 0.01:
                    result.add_warning(
                        f"{context_prefix}Very low standard deviation: {stdev_val:.6f}"
                    )
                elif stdev_val > 5.0:
                    result.add_warning(
                        f"{context_prefix}Very high standard deviation: {stdev_val:.4f}"
                    )
                ...
                result.add_warning(
                    f"{context_prefix}Could not calculate statistics: {str(e)}"
                )
    ...
    def validate_embedding_batch(
        self, embeddings: List[List[float]], context: str = ""
    ) -> ValidationResult:
        """
        ...
        if invalid_count > 0:
            ...
            if invalid_count > 5:
                result.add_error(
                    f"Total invalid embeddings in batch: {invalid_count}/{len(embeddings)}"
                )
        ...
        if embeddings and all(isinstance(emb, list) for emb in embeddings):
            ...
            if len(unique_dimensions) > 1:
                result.add_error(
                    f"Inconsistent dimensions in batch: {unique_dimensions}",
    ...
    def validate_similarity_scores(self, scores: List[float]) -> ValidationResult:
        ...
        for i, score in enumerate(scores):
            if not isinstance(score, (int, float)):
                result.add_error(
                    f"Non-numeric similarity score at index {i}", "NON_NUMERIC_SCORE"
                )
                continue
        ...
        if out_of_range:
            result.add_error(
                f"Similarity scores outside [0,1] range: {len(out_of_range)} instances",
                "SCORES_OUT_OF_RANGE",
            )
        ...
        if len(scores) > 1:
            ...
            if non_descending:
                result.add_warning(
                    f"Similarity scores not in descending order at positions: {non_descending[:5]}"
                )
